...
async function fillDetailsAndReachDeposit(
  ...
  await user.click(
    await screen.findByRole("button", { name: "09:00 — Available" }),
  );
...
describe("BookingWizard — deposit step", () => {
  it("recovers an active server-held attempt without creating a second PaymentIntent", async () => {
    sessionStorage.setItem(
      "comaz:public-booking-recovery:test-garage",
    ...
    await screen.findByRole("heading", { name: "Review" });
    expect(api.recoverDepositAttempt).toHaveBeenCalledWith(
      "test-garage",
      "opaque-recovery-token",
    );
  ...
  it("shows a server-confirmed recovered payment as complete without asking to pay again", async () => {
    ...
    expect(api.createDepositIntent).not.toHaveBeenCalled();
    expect(
      screen.queryByRole("button", { name: /Pay deposit/ }),
    ).not.toBeInTheDocument();
    expect(
      sessionStorage.getItem("comaz:public-booking-recovery:test-garage"),
    ).toBeNull();
  });
  ...
  it("drops an expired recovery capability and never resurrects its hold", async () => {
    ...
    vi.mocked(api.recoverDepositAttempt).mockResolvedValue({
      booking_request_id: "br-expired",
      booking_reference: "BKEXP",
      status: "EXPIRED",
  ...
  it("is skipped entirely when the selected service has no deposit", async () => {
    vi.mocked(api.getPublicGarage).mockResolvedValue({
      ...GARAGE,
      appointment_types: [
        { ...GARAGE.appointment_types[0], deposit_required: false },
      ],
    });
    ...
    await screen.findByText("Test Garage");
    await user.click(
      await screen.findByRole("button", { name: /Full Service/ }),
    );
    await user.click(
      await screen.findByRole("gridcell", {
        name: /10 September 2026 — Good availability, selectable/,
  ...
  it("shows the deposit amount and remaining balance", async () => {
    ...
    expect(
      screen.getAllByText("£20.00", { exact: false }).length,
    ).toBeGreaterThan(0);
    expect(screen.getByText(/Deposit due now/)).toBeInTheDocument();
    expect(screen.getByText("£80.00")).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Pay deposit/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Apple Pay or Google Pay may also appear/),
    ).toBeInTheDocument();
  });
  ...
  it('shows a plain "slot unavailable" message on an ordinary 409', async () => {
    vi.mocked(api.createDepositIntent).mockRejectedValue(
      new ApiError(
        { code: 409, status: "Conflict", message: "Full up." },
        "fallback",
      ),
    );
    ...
    await screen.findByText(
      "This time is no longer available. Please go back and choose another.",
    );
  });
