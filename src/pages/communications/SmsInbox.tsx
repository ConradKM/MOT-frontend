...
import { formatPhoneDisplay, formatRecentTimestamp, smsStatusLabel } from '../../lib/communications'
  ...
  return (
    ...
    >
      ...
      {conversation.customer && (
        <p className="text-xs text-slate-400">{formatPhoneDisplay(conversation.phone)}</p>
      )}
      <div className="flex items-center justify-between gap-2">
...
function MessageBubble({ message }: { message: CommunicationLog }) {
  ...
  const statusText = message.status_detail || smsStatusLabel(message.status)
  return (
...
function NewSmsModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const phone = result.to_address ?? targetPhone
      if (isFailedSend(result.status)) {
        setError(result.status_detail || result.error_message || "That SMS wasn't sent.")
        return
      }
      if (result.status === 'SKIPPED_NOT_CONFIGURED') {
        showToast('Recorded, but SMS isn’t connected so it wasn’t delivered.')
      } else {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        ...
        <label className="mt-3 block text-xs font-medium text-slate-500" htmlFor="new-sms-body">
          Message
        </label>
        <textarea
          id="new-sms-body"
...
/** SMS inbox - deliberately simpler than WhatsAppInbox: no archive/delete
 * (SMS has no equivalent conversation-state table yet) and no automation
 * takeover (the conversation engine is WhatsApp/voice-only). Automated
 * booking-event SMS (see app/communications/sms_automation.py) appear in
 * the same thread as a manual send - they go through the same
 * send_sms_message/CommunicationLog path, not a separate system. */
export function SmsInbox() {
  ...
  const { data: conversationsData, isLoading: conversationsLoading } = useSmsConversations({
    search,
  })
  const { data: thread, isLoading: threadLoading } = useSmsConversationMessages(
    selectedPhone ?? undefined,
  )
  const markRead = useMarkSmsConversationRead()
  const sendMessage = useSendSmsMessage()
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (isFailedSend(result.status)) {
        showToast(result.error_message || "That SMS wasn't sent.")
        return
      }
      setDraft('')
    } catch (err) {
  ...
  const smsConnected = overview?.capabilities.sms_configured ?? false

  return (
    <div>
      {!smsConnected && (
        <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          SMS is not connected yet - messages can still be reviewed here, but sending will be
          ...
          <div className="flex-1 overflow-y-auto">
            {conversationsLoading ? (
              ...
              <p className="px-4 py-6 text-sm text-slate-500">
                No SMS conversations yet. They'll appear here as customers text you, or once a
                booking confirmation/reminder is sent.
              </p>
            ) : (
              conversations.map((c) => (
                <ConversationRow
            ...
            <>
              <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500">{formatPhoneDisplay(selectedPhone)}</p>
                </div>
                {!thread?.customer && (
                  <Link
                    to={`/${garageId}/customers/new?phone=${encodeURIComponent(selectedPhone)}`}
                    className="shrink-0 text-sm font-medium text-slate-600 hover:underline"
                  >
                    Add customer
                  </Link>
                )}
